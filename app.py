...
@app.route("/find/<text>/<intsura>", methods=['GET'])
@cross_origin(allow_headers=['Content-Type'])
def findSura(text,intsura):
    cv = intsura.split("-")
    conn = mysql.connect()
    cur = conn.cursor(pymysql.cursors.DictCursor)
    cur.execute("SELECT * FROM search WHERE search = %s and type = %s",(text,json.dumps(cv, ensure_ascii=False)))
    search = cur.fetchall()
    if (len(search) == 0):
        ss = '%' + text + '%'
        conn = mysql.connect()
        cur = conn.cursor(pymysql.cursors.DictCursor)
        cur.execute("SELECT * FROM quran_simple WHERE ((aya LIKE %s) AND (suraId IN %s))",(ss,cv))
        quran = cur.fetchall()
        simL = searchOp(text,quran)
        maxL(simL)
        res = {'sim':simL,'quran':quran}
        save2(text,quran,simL,cv)
        return json.dumps(res, ensure_ascii=False)
    else :
        res = {'sim':search[0]['sim'],'quran':search[0]['result']}
        return json.dumps(res, ensure_ascii=False)
...
def findSave():
    conn = mysql.connect()
    cur = conn.cursor(pymysql.cursors.DictCursor)
    cur.execute("SELECT id,search,type FROM search")
    search = cur.fetchall()
    res = []
    for val in search :
        if (val['type'] == None):
            res.append({'id':val['id'],'search':val['search'],'type':val['type']})
        else :
            d = json.JSONDecoder()
            t = d.decode(val['type'])
            type = []
            for x in t :
                if (x != "") :
                    type.append(int(x))
            res.append({'id':val['id'],'search':val['search'],'type':type})
    return json.dumps(res, ensure_ascii=False)

@app.route("/get-find/<id>", methods=['GET'])
@cross_origin(allow_headers=['Content-Type'])
def getFind(id):
    conn = mysql.connect()
    cur = conn.cursor(pymysql.cursors.DictCursor)
    cur.execute("SELECT * FROM search WHERE id = %s",id)
    search = cur.fetchone()
    d = json.JSONDecoder()
    quran = d.decode(search['result'])
    sim = d.decode(search['sim'])
    res = {'sim':sim,'quran':quran}
    return json.dumps(res, ensure_ascii=False)
...
def find(text):
    conn = mysql.connect()
    cur = conn.cursor(pymysql.cursors.DictCursor)
    cur.execute("SELECT * FROM search WHERE search = %s",text)
    search = cur.fetchall()
    if (len(search) == 0):
        ss = '%' + text + '%'
        conn = mysql.connect()
...
def searchOp(text,quran):
    ...
    maxL(liste2)
    if(len(liste) != 0):
        nextL.append([len(liste[0])-1,liste[0][1][2],color[idC],1])
        if(idC == 123):
            idC = 0
        else :
            idC += 1
        for j in range (1,len(liste)) :
            i = 0
            while True :
                if(nextL[i][1].find(liste[j][1][2]) != -1):
                    break
                else :
                    i += 1
                if (i == len(nextL)) :
                    nextL.append([len(liste[j])-1,liste[j][1][2],color[idC],1])
                    if(idC == 123):
                        idC = 0
                    else :
                        idC += 1
                    break
    if(len(liste2) != 0):
        lastL.append([len(liste2[0])-1,liste2[0][1][2],color[idC],-1])
        if(idC == 123):
            idC = 0
        else :
            idC += 1
        for j in range (1,len(liste2)) :
            i = 0
            while True :
                if(lastL[i][1].find(liste2[j][1][2]) == len(lastL[i][1])-len(liste2[j][1][2])):
                    break
                else :
                    i += 1
                if (i == len(lastL)) :
                    lastL.append([len(liste2[j])-1,liste2[j][1][2],color[idC],-1])
                    if(idC == 123):
                        idC = 0
                    else :
                        idC += 1
    ...
    if (len(res) != 0) :
        ...
        for k in res :
            if (k[2] > m) :
                m = k[2]
        result.append(m)
        result.append([a,b,str[len(str)-m:len(str)]])
        for k in res :
            if (k[2] == m):
                result.append([k[0],k[1]])
    return result
...
@app.route("/search/<text>/<str>", methods=['GET'])
@cross_origin(allow_headers=['Content-Type'])
def search(text,str):
    ss = '%' + text + '%'
    conn = mysql.connect()
